...
@section('controller')
<div c id="dash" >
    <div class="wrapper bg-white shadow p-3 mb-5 bg-body rounded mt-5" >
            ...
            <div class="comment d-flex align-items-start mt-2" >
                ...
                <div class="d-flex flex-column ">
                    ...
                    <p class="text-muted">
                        ...
                        <span class="date">{{\Carbon\Carbon::parse($item->done_time)->format('j F, Y')}}</span>
                    </p>
                </div>
                ...
                @else
                    <form action="/completed/{{$item['id']}}" method="post">
                        @csrf
                        <a href="{{ route('completed.io', $item->id)}}" class="fas fa-circle-check text-primary btn"></a>
                    </form>
                </div>
                <div class="d-flex flex-column ">
                    {{--path yang {id} dikirim data dinamis (data dari db) makanya disitu pake {{}}--}}
